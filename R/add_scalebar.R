...
#'
#' This function adds a scalebar to frames created with \code{\link{frames_spatial}}.
#'
...
add_scalebar <- function(frames, distance = NULL, height = 0.015, position = "bottomleft", x = NULL, y = NULL, colour = "black", label_margin = 1.2, units = "km", verbose = TRUE) {
  ...
  .corner <- function(xy) {
    list(
      bottomleft = c(min(xy$xmin), min(xy$ymin)), upperleft = c(min(xy$xmin), max(xy$ymax)),
  ...
  gg.dist <- list(
    x = as.numeric(suppressPackageStartupMessages(st_distance(gg.corner_sf$bottomleft, gg.corner_sf$bottomright, by_element = T))) / 1000,
  ...
  scale.outer <- rbind.data.frame(
    scale.outer, c(scale.outer[1], (scale.outer[2] + (gg.diff$y * height))),
  ...
  text.data <- cbind.data.frame(
    x = c(min(scale.outer$x), min(scale.inner$x), max(scale.outer$x)),
  ...
  add_gg(frames,
    gg = expr(list(
      geom_polygon(aes_string(x = "x", y = "y"), data = scale.outer, fill = "white", colour = "black"),
