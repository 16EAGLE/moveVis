...
view_spatial <- function(m, render_as = "mapview", time_labels = TRUE, stroke = TRUE, path_colours = NA, path_legend = TRUE,
  ...
  if (is.character(render_as)) {
    ...
    out("Argument 'render_as' must be of type 'character'.", type = 3)
  }
  ...
  if (render_as == "mapview") {
    ...
    map <- mapview::mapview(m.df,
      map.types = "OpenStreetMap", xcol = "x", ycol = "y", zcol = "name", legend = path_legend,
  ...
  if (render_as == "leaflet") {
    ...
    for (i in 1:length(m.split)) {
      map <- leaflet::addCircleMarkers(
        map = map, lng = m.split[[i]]$x, lat = m.split[[i]]$y,
    ...
    map <- leaflet::addScaleBar(map = leaflet::addLegend(
      map = map, colors = unique(m.df$colour),
      labels = unique(m.df$name), opacity = 1, title = path_legend_title
    ), position = "bottomleft")
  }
  return(map)
}
