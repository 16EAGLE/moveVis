...
#'    \item \code{\link{get_maptypes}} returns a character vector of available map types that can be used with the \code{frames_*} function familiy. \code{moveVis} supports OpenStreetMaps and Mapbox basemap imergay. Alternatively, you can provide custom imagery to \code{\link{frames_spatial}}.
...
#'    \item \code{\link{add_gg}} adds \code{ggplot2} functions (e.g. to add layers such as points, polygons, lines, or to change scales etc.) to the animation frames created with the \code{frames_*} function familiy. Instead of creating your own \code{ggplot2} functions, you can use one of the other moveVis \code{add_} functions:
#'    \item \code{\link{add_labels}} adds character labels such as title or axis labels to animation frames created with the \code{frames_*} function familiy.
#'    \item \code{\link{add_scalebar}} adds a scalebar to the animation frames created with the \code{frames_*} function familiy.
#'    \item \code{\link{add_northarrow}} adds a north arrow to the animation frames created with the \code{frames_*} function familiy.
#'    \item \code{\link{add_progress}} adds a progress bar to animation frames created with the \code{frames_*} function familiy.
#'    \item \code{\link{add_timestamps}} adds timestamps to animation frames created with the \code{frames_*} function familiy.
#'    \item \code{\link{add_text}} adds static or dynamically changing text to the animation frames created with the \code{frames_*} function familiy.
#'    \item \code{\link{add_colourscale}} adjusts the colour scales of the animation frames created with the \code{frames_*} function familiy and custom map imagery.
...
#'    \item \code{\link{animate_frames}} creates an animation from a list of frames computed with the \code{frames_*} function familiy.
#' }
#' 
